...
- `make test-e2e` - запустить E2E тесты локально (dockertest)
- `make test-e2e-isolated` - запустить E2E тесты в изолированном окружении (docker-compose)
- `make test-e2e-up` - поднять E2E окружение без запуска тестов (отладка)
- `make test-e2e-down` - остановить E2E окружение
...
#### 1. Изолированное окружение (рекомендуется для CI/CD)

E2E тесты запускаются в отдельном docker-compose окружении с собственной базой данных и сетью:
...
**Преимущества:**
- ✅ Полная изоляция от production окружения
- ✅ Отдельная БД для тестов (`testdb` на порту 5433)
- ✅ Отдельный API контейнер (порт 8082)
- ✅ Автоматическая очистка после тестов
- ✅ Подходит для CI/CD пайплайнов

#### 2. Локальное тестирование с dockertest

Для быстрой разработки и отладки:

```bash
# Локальный запуск с автоматическим поднятием БД
make test-e2e
...
- Использует `dockertest` для автоматического управления контейнерами
